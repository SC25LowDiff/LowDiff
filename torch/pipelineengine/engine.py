...
def diff_ckpt_saver(queue,save_batch_freq):
    ...
    while True:
        ...
        diff_cpu = _to_cpu(diff)
    
        if save_batch_freq == 1 :
            begin = time.time()
            torch.save(diff_cpu, filename)
            end = time.time()
            print("Saved {} time: {:.3f}s".format(filename, end - begin))
        
        else: 
            batch_buffer[i] = diff_cpu
...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        ...
        # differential ckpt
        self.i = 0
        self.diff_ckpt = [{} for _ in range(torch.cuda.device_count())]
        self.queue = mp.Queue()
        self.save_process = mp.Process(target=diff_ckpt_saver, args=(self.queue,1))
    ...
    def train_batch(self, data_iter=None):
        ...
        self.i += 1
        # print("finish training iteration {}".format(self.i))
